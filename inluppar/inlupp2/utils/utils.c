/*
* UTILS VERSION 5.0
* utils.c and utils.h, is for lab 5.
*/
...
bool check_valid_menu(char *c) {
    if (!not_empty(c) || !is_digit(c[0])) {
        return true;
    }
    return false;
}
...
char ask_question_menu(char *question) {
    return ask_question(question, check_valid_menu, (convert_func) menu_c).c;
}
